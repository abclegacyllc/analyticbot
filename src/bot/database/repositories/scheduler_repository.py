...
class SchedulerRepository:
    ...
    async def create_scheduled_post(
        ...
        """
        Creates a new post in the database, now with optional media fields.
        """
        async with self.pool.acquire() as conn:
            return await conn.fetchval(
                """
                INSERT INTO scheduled_posts (channel_id, text, file_id, file_type, schedule_time)
                VALUES ($1, $2, $3, $4, $5)
                RETURNING post_id
                """,
                channel_id, text, file_id, file_type, schedule_time
            )
    ...
    async def update_post_status(self, post_id: int, status: str):
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE scheduled_posts SET status = $1, sent_at = CASE WHEN $1 = 'sent' THEN NOW() ELSE sent_at END WHERE post_id = $2",
                status, post_id
            )
        ...
        async with self.pool.acquire() as conn:
            return await conn.fetch(
                ...
                SELECT
                    sp.post_id,
                    sp.text,
                    sp.file_id, -- Include file_id in the selection
                    sp.file_type, -- Include file_type
                    sp.schedule_time,
                    c.channel_name
